...
def prod_plot(df, column, color_scale):
    ...
    fig.update_layout(
        ...
        margin={'t': 80},  # Adjusted top margin to ensure title visibility
    )
    # Show the figure
    return fig
...
col2.plotly_chart(go.Figure(go.Bar(y=top10_chart['Country'], x=top10_chart[chart_option], orientation='h')).update_layout(title=graph_title('Top 10 countries'), margin={'t': 80}),
                  use_container_width=True)

# Creates a lineplot depending on the option selected
if chart_option == 'Imports':
    lineplot_imports(line_option)
else:
    lineplot_production(line_option)
